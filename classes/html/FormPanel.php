...
final class FormPanel extends \html\Panel {
	...
	public function select($name, $label, $required, array $options) {
		$this->setNodeId();
		...
		foreach ($options as $value => $txt) {
			$selected = "";
			if (isset($_POST[$this->prevSelect['name']]) && $_POST[$this->prevSelect['name']] === $value) {
				$selected = "selected";
			}
	...
	public function file($name, $label) {
		$this->setNodeId();
		$this->inputs[$this->node_id] = $this->inputBlock($name, $label, "<input class=\"input-file\"
	...
	public function textarea($name, $label) {
		$this->setNodeId();
		$this->inputs[$this->node_id] = $this->inputBlock($name, $label, "<textarea class=\"form-control\"
	...
	public function checkbox($name, $label) {
		$this->setNodeId();
		$this->inputs[$this->node_id] = $this->inputBlock($name, $label, "<div class=\"checkbox\">
	...
	public function button($name, $label, $class) {
		$this->setNodeId();
		$this->inputs[$this->node_id] = $this->inputBlock($name, "", "<button class=\"$class\"
                    id=\"$name\"
